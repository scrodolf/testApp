...
/// Holds information about a unit override for a specific product.
class UnitOverrideDetail {
  UnitOverrideDetail({required this.unit, required this.factorToBase});

  final Unit unit;
  final double factorToBase;
}

/// Aggregates a [Product] with its default serving unit, category values and
/// any unit overrides.
...
class ProductDao {
  ...
  Future<List<ProductWithDetails>> getAllProducts() async {
    ...
    for (final row in productRows) {
      ...
      final overrideRows = await _db.select(_db.productUnitOverrides).join([
        innerJoin(
            _db.units, _db.units.id.equalsExp(_db.productUnitOverrides.unitId)),
      ])
        ..where(_db.productUnitOverrides.productId.equals(product.id));
      final overrideFetched = await overrideRows.get();
      final overrides = overrideFetched
          .map((r) => UnitOverrideDetail(
                unit: r.readTable(_db.units),
                factorToBase:
                    r.readTable(_db.productUnitOverrides).factorToBase,
              ))
          .toList();
...
=======
  ...
  Future<ProductWithDetails?> getProductById(int id) async {
    ...
    final overrides = overrideFetched
        .map((r) => UnitOverrideDetail(
              unit: r.readTable(_db.units),
              factorToBase: r.readTable(_db.productUnitOverrides).factorToBase,
            ))
        .toList();
